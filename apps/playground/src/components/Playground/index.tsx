...
function Playground() {
    ...
    const addLog = (type: ConsoleLog['type'], ...args: any[]) => {
      ...
      const formatArg = (arg: any): string => {
        ...
        if (typeof arg === 'object') {
          try {
            // Check if it's an event object with specific structure
            if (arg.eventType || arg.handler || arg.payload) {
      ...
      // Build message - first arg is usually a label, rest are data
      let message = '';
      if (args.length > 0) {
        const firstArg = formatArg(args[0]);
        if (args.length === 1) {
          message = firstArg;
        } else {
          // If first arg is a string label, format it nicely
          if (typeof args[0] === 'string') {
            message = `${args[0]}\n${args.slice(1).map(formatArg).join('\n')}`;
          } else {
  ...
  const handleEvent = (eventType: string, handler: string, payload?: unknown) => {
    // Log event in a clear, structured format
    const eventDetails = {
      eventType,
      handler,
    ...
    console.log('Event triggered:', eventDetails);
                    ...
                    <div key={log.id} className="console-log" data-type={log.type}>
                      ...
                      >
                        {log.message.split('\n').map((line, idx) => {
                          ...
                          return (
                            <div key={idx} className={isJson ? 'console-log-json' : 'console-log-line'}>
                              {isJson ? (
                                <pre className="console-json-content">{line}</pre>
                              ) : (
                                line
                              )}
                            </div>
